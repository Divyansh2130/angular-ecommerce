...
import {
  ...
  getProductsByTypeAndCategory as getProductsByTypeAndCategoryService,
...
export const getProducts = async (req: Request, res: Response) => {
  try {
    const limit = Math.min(getQueryNumber(req.query.limit, 200), 500);
...
export const getProductsByTypeAndCategory = async (req, res) => {
  try {
    ...
    const data = await getProductsByTypeAndCategoryService({ categoryId, typeId, limit, page });
